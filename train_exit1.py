...
def parse_args():
    ...
    parser.add_argument('--exit', default=1, type=int)
...
def run_training(args):
    ...
    for i in range(len(keep_masks_exit)):
        temp = keep_masks_exit[i]
        size_ = temp.size()
        random_ = torch.from_numpy(np.random.choice([0, 1], size=size_, p=[1./2, 1./2])).cuda()
        keep_masks_exit[i] = temp * random_

    temp_masks = [a - b for a, b in zip(keep_masks[exit_idx:], keep_masks_exit[exit_idx:])]
    final_keep_masks = keep_masks[0:exit_idx] + keep_masks_exit[exit_idx:] + temp_masks

    model = Wide_ResNet_1(28, 10, 0.0, 100)
    model = model.cuda()
    ...
    for current_epoch in range(0, args.epoch):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            if args.mode == 'CE':
                ...
                output_e =  (output1/2 + output2/2).detach()
...
def validate(args, test_loader, model, criterion, current_epoch=0):
    ...
    for i, (input, target) in enumerate(test_loader):
        ...
        prece = accuracy((output1+output2).data, target, topk=(1,))
